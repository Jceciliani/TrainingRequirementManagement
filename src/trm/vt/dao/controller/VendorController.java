...
import trm.vt.dao.vendorTrainer.VendorTrainer;
import trm.vt.dao.vendorTrainer.VendorTrainerDAO;
...
/*		List<VendorTrainingRequestAndStatus> list330 = new VendorTrainingRequestAndStatusDAO()
		...
		/*System.out.println("-----------------------" + list330.toString());*/
		...
		System.out.println(itc.toString() + itc.size());
...
/*	@RequestMapping(value ="updateTrainer/{dtt_trainer_request_id}")
	...
	public String update(HttpServletRequest request,  @ModelAttribute("TrainingSchedule") trm.dt.dao.trainingSchedule.TrainingSchedule s, @ModelAttribute("TrainingRequest") TrainingRequest req,
		...
		new ExecutiveWorkflowDAO().insertTempExecutiveWorkflow(training_request_id);
		
		new CallbackFunction().statusChange(training_request_id, 230, firstName, lastName);
	...
	public @ResponseBody byte[] getLogs(@RequestParam("id") int id, ModelMap map)
			throws JsonGenerationException, JsonMappingException, IOException {
		int training_request_id = new TrainingRequestDAO().getTrainingRequestIdWithVendorTrainingRequestId(id);
		List<trm.vt.dao.trainingRequestLog.TrainingRequestLog> logs = new trm.vt.dao.trainingRequestLog.TrainingRequestLogDAO()
				.getTrainingRequestLog(training_request_id);
	...
	public String insertTrainingScheduleForm(@PathVariable("id") int id, HttpServletRequest request) {
		...
		new VendorTrainingRequestDAO().updateVendorTrainingRequestWithScheduleId(schedule_id, id);
		int training_request_id = new TrainingRequestDAO().getTrainingRequestIdWithVendorTrainingRequestId(id);
		new CallbackFunction().statusChange(training_request_id, 330, firstName, lastName);
		String rtrn = "redirect:/dashboard#myModal" + Integer.toString(id);
		return rtrn;
	}
	...
	@RequestMapping(value="vendormanagement/deleteVendor", method = RequestMethod.POST)
    public String deleteVendor(@RequestParam(value = "rowid") String rowid, HttpServletRequest request, ModelMap model) {
    	System.out.println("In deleteVendor controller. The row id is " + rowid);
    	
    	new VendorDetailDAO().deleteVendorDetail(Integer.parseInt(rowid));
    	
    	return "redirect:/vendormanagement/0";
    }
     
     
    @RequestMapping(value="vendormanagement/trainerList", method = RequestMethod.GET)
    @ResponseBody
    public byte[] trainerList(@RequestParam(value = "rowid") String rowid, HttpServletRequest request, ModelMap map) throws JsonGenerationException, JsonMappingException, IOException {
    	
    	System.out.println("In trainerList controller. Id is " + rowid);
    	
    	List<VendorTrainer> vendorTrainers = new VendorTrainerDAO().getAllTrainersforVendor(Integer.parseInt(rowid));
	    ...
	    final ByteArrayOutputStream out = new ByteArrayOutputStream();
	    final ObjectMapper mapper = new ObjectMapper();
	    mapper.writeValue(out, vendorTrainers);
	    final byte[] JSON = out.toByteArray();
	    System.out.println(new String(JSON));
		return JSON;
    }
	...
	// Get Progress/Status of the Vendor Training Request
	@RequestMapping(value = "/DTLogs", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody byte[] getDTLogs(@RequestParam("id") int id, ModelMap map) throws JsonGenerationException, JsonMappingException, IOException {
		List<trm.vt.dao.trainingRequestLog.TrainingRequestLog> logs = new trm.vt.dao.trainingRequestLog.TrainingRequestLogDAO()
				.getTrainingRequestLog(id);
		System.out.println("THE LOGS ARE --------------------- " + logs.toString());

		map.addAttribute("developLogs", logs);
	...
	// Get Progress/Status of the Vendor Training Request
	@RequestMapping(value = "/ITLogs", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody byte[] getITLogs(@RequestParam("id") int id, ModelMap map) throws JsonGenerationException, JsonMappingException, IOException {
		
		int training_request_id = new TrainingRequestDAO().getTrainingRequestIdWithInternalTrainingRequestId(id);
		List<trm.vt.dao.trainingRequestLog.TrainingRequestLog> logs = new trm.vt.dao.trainingRequestLog.TrainingRequestLogDAO()
				.getTrainingRequestLog(training_request_id);
		System.out.println("THE LOGS ARE --------------------- " + logs.toString());

		map.addAttribute("internalLogs", logs);
